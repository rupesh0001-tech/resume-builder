...
const ThemeSelector = () => {
  ...
  return (
    <div className="flex flex-col w-48 text-sm relative py-2 cursor-pointer ">
      <button
        type="button"
        onClick={() => setIsOpen(!isOpen)}
        className="group flex items-center justify-between w-48 text-left px-2 py-2 border rounded bg-white text-gray-700 border-gray-300 shadow-sm hover:bg-gray-50 focus:outline-none"
      >
      ...
      {isOpen && (
        <ul className="w-48 bg-white border border-gray-300 rounded shadow-md mt-1 py-2 right-0">
          {temps.map((temp) => (
            <li
              key={temp.name}
